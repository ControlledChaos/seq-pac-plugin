<?php
/**
 * Sequoia Pacific Realty Plugin
 *
 * @package     Sequoia_Pacific_Realty_Plugin
 * @version     1.0.0
 * @author      Greg Sweet <greg@ccdzine.com>
 * @copyright   Copyright © 2018, Greg Sweet
 * @link        https://github.com/ControlledChaos/seq-pac-plugin
 * @link        https://controlledchaos.github.io/seq-pac-plugin/
 * @license     GPL-3.0+ http://www.gnu.org/licenses/gpl-3.0.txt
 *
 * Plugin Name:  Sequoia Pacific Realty Plugin
 * Plugin URI:   https://github.com/ControlledChaos/seq-pac-plugin
 * Description:  WordPress/ClassicPress plugin for the Sequoia Pacific Realty website.
 * Version:      1.0.0
 * Author:       Controlled Chaos Design
 * Author URI:   http://ccdzine.com/
 * License:      GPL-3.0+
 * License URI:  https://www.gnu.org/licenses/gpl.txt
 * Text Domain:  seq-pac-plugin
...
if ( ! class_exists( 'Sequoia_Pacific_Realty_Plugin' ) ) :
	final class Sequoia_Pacific_Realty_Plugin {
	 ...
	 *              Change to `self` if used.
		...
		private function constants() {
			 ...
			 */
			if ( ! defined( 'SPP_DOMAIN' ) ) {
				define( 'SPP_DOMAIN', 'seq-pac-plugin' );
			}
			 ...
			 * Change the second part of the define(), here as 'seq-pac-plugin',
			 * to your preferred page slug.
			 *
			 * @since  1.0.0
			 * @return string Returns the URL slug of the admin pages.
			 */
			if ( ! defined( 'SPP_ADMIN_SLUG' ) ) {
				define( 'SPP_ADMIN_SLUG', 'seq-pac-plugin' );
			}
			 ...
			 */
			if ( ! defined( 'SPP_DEFAULT_META_IMAGE' ) ) {
				define(
					'SPP_DEFAULT_META_IMAGE',
		...
		public function __clone() {
			...
			_doing_it_wrong( __FUNCTION__, __( 'This is not allowed.', 'seq-pac-plugin' ), '1.0.0' );
		...
		private function dependencies() {
			...
			require_once SPP_PATH . 'includes/class-init.php';

			// Include the activation class.
			require_once SPP_PATH . 'includes/class-activate.php';

			// Include the deactivation class.
			require_once SPP_PATH . 'includes/class-deactivate.php';
	 ...
	 * @since  1.0.0
	 * @access public
	 * @return object Returns the instance of the `Sequoia_Pacific_Realty_Plugin` class.
	 */
	function spp_plugin() {

		return Sequoia_Pacific_Realty_Plugin::instance();
 ...
 * @return void
 */
register_activation_hook( __FILE__, '\spp_activate_plugin' );
register_deactivation_hook( __FILE__, '\spp_deactivate_plugin' );
 ...
 * @return void
 */
function spp_deactivate_plugin() {
...
function spp_about_link( $links ) {
	...
	if ( is_admin() ) {
		...
		if ( spp_acf_options() ) {
			...
			// Get the field.
			$acf_position = get_field( 'spp_site_plugin_link_position', 'option' );
		...
		// If ACF is not active, get the field from the WordPress options page.
			...
			// Get the field.
			$position = get_option( 'spp_site_plugin_link_position' );
		}

		if ( true == $position ) {
			$url = admin_url( 'index.php?page=' . SPP_ADMIN_SLUG . '-settings' );
		} else {
			$url = admin_url( 'admin.php?page=' . SPP_ADMIN_SLUG . '-settings' );
		}
...
add_filter( 'plugin_action_links_' . plugin_basename( __FILE__ ), 'spp_about_link' );
 ...
 * @link   https://codex.wordpress.org/Plugin_API/Filter_Reference/plugin_action_links_(plugin_file_name)
 */
function spp_settings_links( $links, $file ) {
	...
	if ( is_admin() ) {
		...
		if ( spp_acf_options() ) {
			...
			// Get the field.
			$acf_position = get_field( 'spp_settings_link_position', 'option' );
		...
		// If ACF is not active, get the field from the WordPress options page.
			...
			// Get the field.
			$position = get_option( 'spp_site_settings_position' );
		}
		...
		if ( $file == plugin_basename( __FILE__ ) ) {
			...
			$links[] = sprintf(
				'<a href="%1s" class="' . SPP_ADMIN_SLUG . '-settings-link">%2s</a>',
				$url,
				esc_attr( 'Site Settings', 'seq-pac-plugin' )
			);
			$links[] = sprintf(
...
add_filter( 'plugin_row_meta', 'spp_settings_links', 10, 2 );
