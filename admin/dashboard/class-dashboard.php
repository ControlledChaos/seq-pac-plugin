...
class Dashboard {
    ...
    public function metaboxes() {
        ...
        if ( spp_acf_options() ) {
            ...
            $hide = get_field( 'spp_dashboard_hide_widgets', 'option' );
        ...
        // If Advanced Custom Fields is not active.
            ...
            // Get options.
            $welcome    = get_option( 'spp_hide_welcome' );
            $gutenberg  = get_option( 'spp_hide_try_gutenberg' );
            $editor     = get_option( 'spp_classic_editor' );
            $wp_news    = get_option( 'spp_hide_wp_news' );
            $quickpress = get_option( 'spp_hide_quickpress' );
            $at_glance  = get_option( 'spp_hide_at_glance' );
            $activity   = get_option( 'spp_hide_activity' );
    ...
    public function add_help() {
		...
		$screen->add_help_tab( [
			...
			'title'    => __( 'Welcome Panel', 'seq-pac-plugin' ),
			'content'  => null,
		...
		$screen->add_help_tab( [
			...
			'title'    => __( 'Dashboard Widgets', 'seq-pac-plugin' ),
			'content'  => null,
	...
	public function help_welcome_panel() {
        ...
        include_once SPP_PATH . 'admin/dashboard/partials/help/help-welcome-panel.php';
	...
	public function help_dashboard_widgets() {
        ...
        include_once SPP_PATH . 'admin/dashboard/partials/help/help-dashboard-widgets.php';
    ...
    public function help_dashboard_sidebar() {
        ...
        $html  = sprintf(
            '<h4>%1s %2s</h4>',
            __( 'Custom Dashboard for', 'seq-pac-plugin' ),
             get_bloginfo( 'name' )
        );
	...
	public function styles() {
        ...
        if ( $screen->id == 'dashboard' ) {
            wp_enqueue_style( SPP_ADMIN_SLUG . '-dashboard', SPP_URL .  'admin/dashboard/assets/css/dashboard.min.css', [], null, 'screen' );
        }
