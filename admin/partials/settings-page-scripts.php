...
$tabs = [
    ...
    sprintf(
        ...
        esc_html__( 'General', 'seq-pac-plugin' )
    ),
    ...
    sprintf(
        ...
        esc_html__( 'Vendor', 'seq-pac-plugin' )
    )

];

// Apply a filter to the tabs array for adding tabs.
$page_tabs = apply_filters( 'spp_tabs_script_options', $tabs );
...
if ( 'general' == $active_tab ) {
    $section = 'spp-scripts-general';
    $fields  = 'spp-scripts-general';
} elseif ( 'vendor' == $active_tab ) {
    $section = 'spp-scripts-vendor';
    $fields  = 'spp-scripts-vendor';
} else {
    $section = null;
    $fields  = null;
}
...
$do_section = apply_filters( 'spp_section_script_options', $section );
$do_fields  = apply_filters( 'spp_fields_script_options', $fields );
 ...
 */
if ( 'general' == $active_tab  ) {
    $save = __( 'Save General', 'seq-pac-plugin' );
} elseif ( 'vendor' == $active_tab ) {
    $save = __( 'Save Vendor', 'seq-pac-plugin' );
} else {
    $save = __( 'Save Settings', 'seq-pac-plugin' );
}

// Apply a filter for new tabs added by another plugin or from a theme.
$button = apply_filters( 'spp_save_script_options', $save );

?>
<div class="wrap">
    <h1 class="wp-heading-inline"><?php esc_html_e( 'Script Options', 'seq-pac-plugin' ); ?></h1>
    <?php if ( is_rtl() ) : ?>
    <p class="description"><?php esc_html_e( 'Script settings from the Controlled Chaos plugin. More information in the Help tab at upper left.', 'seq-pac-plugin' ); ?></p>
    <?php else : ?>
    <p class="description"><?php esc_html_e( 'Script settings from the Controlled Chaos plugin. More information in the Help tab at upper right.', 'seq-pac-plugin' ); ?></p>
    <?php endif; ?>
