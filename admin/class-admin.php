...
class Admin {
	...
	private function dependencies() {
		...
		require_once SPP_PATH . 'admin/class-admin-menu.php';

		// Add menus to the admin toolbar.
		require_once SPP_PATH . 'admin/class-admin-toolbar-menus.php';
	...
	public function admin_footer() {
		...
		if ( spp_classicpress() ) {
			$default = __( 'website powered by ClassicPress', 'seq-pac-plugin' );

		// If the site is running WordPress.
		} else {
			$default = __( 'website powered by WordPress', 'seq-pac-plugin' );
		}
		...
		if ( spp_acf_options() ) {
			 ...
			 */
			$credit = get_field( 'spp_admin_footer_credit', 'option' );
			$link   = get_field( 'spp_admin_footer_link', 'option' );
			...
			if ( $credit && $link ) {
				$footer = sprintf(
					...
					$site,
					esc_html__( 'website designed & developed by', 'seq-pac-plugin' ),
					esc_url( $link ),
					$credit
				);
		 ...
		 */
			...
			$credit = sanitize_text_field( get_option( 'spp_footer_credit' ) );
			$link   = esc_url_raw( get_option( 'spp_footer_link' ) );
		...
		$admin_footer = apply_filters( 'spp_admin_footer', $footer );
	...
	public function enqueue_styles() {
		 ...
		 * @since 1.0.0
		 */
		wp_enqueue_style( SPP_ADMIN_SLUG . '-admin', SPP_URL . 'admin/assets/css/admin.min.css', [], SPP_VERSION, 'all' );
		 ...
		 * @since 1.0.0
		 */
		wp_enqueue_style( SPP_ADMIN_SLUG . '-tooltips', SPP_URL . 'admin/assets/css/tooltips.min.css', [], SPP_VERSION, 'all' );
		 ...
		 */
		if ( spp_acf() ) {
			wp_enqueue_style( SPP_ADMIN_SLUG . '-acf', SPP_URL . 'admin/assets/css/acf.css', [], SPP_VERSION, 'all' );
		}
		 ...
		 * @since 1.0.0
		 */
		$welcome = get_option( 'spp_custom_welcome' );
		if ( $welcome ) {
			wp_enqueue_style( SPP_ADMIN_SLUG . '-welcome', SPP_URL . 'admin/assets/css/welcome.css', [], SPP_VERSION, 'all' );
		}
	...
	public function get_search_form() {
		...
		require SPP_PATH . 'admin/partials/searchform.php';
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( SPP_ADMIN_SLUG . '-conditionalize', SPP_URL . 'admin/assets/js/admin.js', [ 'jquery' ], SPP_VERSION, true );

		// Enqueue scripts for backend functionality of this plugin.
		wp_enqueue_script( SPP_ADMIN_SLUG . '-admin', SPP_URL . 'admin/assets/js/conditionize.flexible.jquery.min.js', [ 'jquery' ], SPP_VERSION, true );
