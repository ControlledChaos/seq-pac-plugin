...
class Filter_By_Template {
	...
	public function filter_dropdown() {
		...
		<select name="page_template_filter" id="page_template_filter">
			<option value="all"><?php _e( 'All Page Templates', 'seq-pac-plugin' ); ?></option>
			<option value="default" <?php echo ( $template == 'default' ) ? ' selected="selected" ' : ''; ?>><?php echo _e( 'Default Template', 'seq-pac-plugin' ); ?></option>
	...
	public function template_columns_head( $columns ) {
		...
		$columns['template'] = __( 'Template', 'seq-pac-plugin' );
	        ...
	        if ( $template ) {
	        	...
	        	if ( $template == 'default' ) {
					...
					echo sprintf(
						...
						__( 'Default Template', 'seq-pac-plugin' ),
						__( 'Default Template', 'seq-pac-plugin' )
					);
				...
				// If it's not the default template.
	        		...
	        		if ( isset( $templates[ $template ] ) ) {
	        			echo sprintf(
							...
							__( 'Template file:', 'seq-pac-plugin' ),
							$template,
							$templates[ $template ]
						);
					...
					// If the template cannot be found.
	        			...
	        			echo sprintf(
							'<span title="%1s">%2s</span>',
							__( 'This template file does not exist', 'seq-pac-plugin' ),
							$template
						);
					}
 ...
 */
function spp_filter_by_template() {
